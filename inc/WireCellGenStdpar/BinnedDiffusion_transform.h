#ifndef WIRECELL_GENSTDPAR_BINNEDDIFFUSION_TRANSFORM
#define WIRECELL_GENSTDPAR_BINNEDDIFFUSION_TRANSFORM
...
#include "WireCellGenStdpar/GdData.h"
...
namespace WireCell 
{
    namespace GenStdpar 
    {
      ...
      /* 	bool operator()(const std::shared_ptr<GenStdpar::GaussianDiffusion>& lhs, const std::shared_ptr<GenStdpar::GaussianDiffusion>& rhs) const; */
      /* }; */
	...
	class BinnedDiffusion_transform 
  {
	public:
      ...
      void get_charge_matrix_stdpar(float* out, size_t dim0, size_t dim1,
                                    std::vector<int>& vec_impact,
                                    const int start_pitch,
                                    const int start_tick);
	
      void get_charge_matrix_stdpar_noscan(float* out, size_t dim0, size_t dim1,
                                           std::vector<int>& vec_impact,
                                           const int start_pitch,
                                           const int start_tick);
	...
	private:
	    ...
	    std::map<int, GenStdpar::ImpactData::mutable_pointer> m_impacts;
