...
GenStdpar::BinnedDiffusion_transform::BinnedDiffusion_transform(const Pimpos& pimpos, const Binning& tbins,
...
bool GenStdpar::BinnedDiffusion_transform::add(IDepo::pointer depo, double sigma_time, double sigma_pitch)
{

    const double center_time = depo->time();
    const double center_pitch = m_pimpos.distance(depo->pos());

    GenStdpar::GausDesc time_desc(center_time, sigma_time);
    {
    ...
    GenStdpar::GausDesc pitch_desc(center_pitch, sigma_pitch);
    {
...
//void GenStdpar::BinnedDiffusion_transform::get_charge_matrix_stdpar(float* out, size_t dim_p, size_t dim_t,
//                                                                    std::vector<int>& vec_impact, const int start_pitch,
//                                                                    const int start_tick)
//{
//  std::cout << "TW_LOG_MESSAGE: Start doing get_charge_matrix_stdpar" << std::endl;
//
//  double wstart, wend ;
...
//  std::cout << "TW_TIMING_MESSAGE: get_charge_matrix_stdpar(): part1 running time : " << g_get_charge_vec_time_part1 << std::endl;
...
//  std::cout << "TW_TIMING_MESSAGE: get_charge_matrix_stdpar(): part2 running time : " << g_get_charge_vec_time_part2 << endl;
//  //FIXME: Is this step really necessary???
//
//  wstart = omp_get_wtime();
//
//  int npatches = m_diffs.size();
//  GenStdpar::GdData* gdata = (GenStdpar::GdData*)malloc(sizeof(GenStdpar::GdData) * npatches);
//
...
//
//  wstart = omp_get_wtime();
//  cout << "pr21 get_charge_matrix_stdpar(): set_sampling_bat() no DtoH time " << wstart - wend << endl;
//  std::cout << "tw: DEBUG: npatches: " << npatches << std::endl;
////  std::cout << "tw: DEBUG: np_vec: " << StdparArray::dump_1d_view(np_d,10000) << std::endl;
////  std::cout << "tw: DEBUG: nt_vec: " << StdparArray::dump_1d_view(nt_d,10000) << std::endl;
////  std::cout << "tw: DEBUG: offsets_d: " << StdparArray::dump_1d_view(offsets_d,10000) << std::endl;
////  std::cout << "tw: DEBUG: patch_idx: " << StdparArray::dump_1d_view(patch_idx,10000) << std::endl;
////  std::cout << "tw: DEBUG: patch_d: " << StdparArray::dump_1d_view(patch_d,10000) << std::endl;
////  std::cout << "tw: DEBUG: qweights_d: " << StdparArray::dump_1d_view(qweights_d,10000) << std::endl;
//
//#pragma omp target teams distribute
//  for(int ip=0; ip<npatches; ip++)
//  {
...
//  }
//  wend = omp_get_wtime();
//  // std::cout << "yuhw: box_of_one: " << StdparArray::dump_2d_view(out,20) << std::endl;
//  // std::cout << "yuhw: DEBUG: out: " << StdparArray::dump_2d_view(out,10000) << std::endl;
//  g_get_charge_vec_time_part3 += wend - wstart;
//  cout << "get_charge_matrix_stdpar(): part3 running time : " << g_get_charge_vec_time_part3 << endl;
//  cout << "get_charge_matrix_stdpar(): set_sampling() running time : " << g_get_charge_vec_time_part4
//       << ", counter : " << counter << endl;
//  cout << "get_charge_matrix_stdpar() : m_fluctuate : " << m_fluctuate << endl;
//
...
////#ifdef HAVE_CUDA_INC
////    cout << "get_charge_matrix_stdpar() CUDA : set_sampling() part1 time : " << g_set_sampling_part1
...
void GenStdpar::BinnedDiffusion_transform::get_charge_matrix(std::vector<Eigen::SparseMatrix<float>* >& vec_spmatrix, std::vector<int>& vec_impact)
{
  const auto ib = m_pimpos.impact_binning();
...
void GenStdpar::BinnedDiffusion_transform::get_charge_matrix_stdpar_noscan(float* out, size_t dim_p, size_t dim_t,
                                                                           std::vector<int>& vec_impact, const int start_pitch,
                                                                           const int start_tick)
{
  std::cout << "TW_LOG_MESSAGE: Start doing get_charge_matrix_stdpar_noscan" << std::endl;

  const auto ib = m_pimpos.impact_binning();

  double wstart, wend, sample_time;
  wstart = omp_get_wtime();
  ...
  //In this version (noscan), we do not calculate index for patch again, but allocate extra memory for each patches. 
  ...
  // decide weight calculation
  // This kernel calculate pvecs and tvecs, each iterator resposible for 1 GD
  // In OpenMP, only parallize the outer loop gives better performance than paralizing both outer/inner loop
  // So we use this pattern in stdpar
  ...
  wend = omp_get_wtime();
  std::cout << "TW_TIMING_MESSAGE: get_charge_matrix_stdpar_noscan(): before set_sampling_bat_noscan takes : " << (wend - wstart) * 1000.0 << " ms" << std::endl;
  
  float* patch = (float*)malloc(sizeof(float) * result);
  
  set_sampling_bat_noscan(npatches, nt_vec, np_vec, pvecs, tvecs, patch, m_normals, gdata);

  std::cout << "TW_TIMING_MESSAGE: get_charge_matrix_stdpar_noscan(): set_sampling_bat_noscan() takes: " << (wstart - wend) * 1000.0 << " ms" << std::endl;
  ...
  wend = omp_get_wtime();
  g_get_charge_vec_time_part3 += wend - wstart;
  std::cout << "TW_TIMING_MESSAGE: get_charge_matrix_stdpar_noscan(): part3 (atomic_add) takes: " << (wend - wstart) * 1000.0 << " ms" << std::endl;
  std::cout << "get_charge_matrix_stdpar_noscan(): Total_ScatterAdd_Time : " << g_get_charge_vec_time_part3 << endl;

  //FIXME: Do not forget to free all variables!
}

// a new function to generate the result for the entire frame ... 
void GenStdpar::BinnedDiffusion_transform::get_charge_vec(std::vector<std::vector<std::tuple<int,int, double> > >& vec_vec_charge, std::vector<int>& vec_impact)
{
...
//We are now working on scan version of this function!
//void GenStdpar::BinnedDiffusion_transform::set_sampling_bat(const unsigned long npatches,  
...
void GenStdpar::BinnedDiffusion_transform::set_sampling_bat_noscan(const unsigned long npatches,  
		const unsigned int* nt_d,
		const unsigned int* np_d, 
		const double* pvecs_d,
		const double* tvecs_d,
...
static
std::pair<double,double> gausdesc_range(const std::vector<GenStdpar::GausDesc> gds, double nsigma)
{
    int ncount = -1;
...
std::pair<double,double> GenStdpar::BinnedDiffusion_transform::pitch_range(double nsigma) const
{
    std::vector<GenStdpar::GausDesc> gds;
    for (auto diff : m_diffs) {
...
std::pair<int,int> GenStdpar::BinnedDiffusion_transform::impact_bin_range(double nsigma) const
{
...
std::pair<double,double> GenStdpar::BinnedDiffusion_transform::time_range(double nsigma) const
{
    std::vector<GenStdpar::GausDesc> gds;
    for (auto diff : m_diffs) {
...
std::pair<int,int> GenStdpar::BinnedDiffusion_transform::time_bin_range(double nsigma) const
{
