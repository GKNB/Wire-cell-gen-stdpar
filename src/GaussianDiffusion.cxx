...
std::vector<double> GenStdpar::GausDesc::sample(double start, double step, int nsamples) const
{
...
std::vector<double> GenStdpar::GausDesc::binint(double start, double step, int nbins) const
{
...
std::vector<double> GenStdpar::GausDesc::weight(double start, double step, int nbins, std::vector<double> pvec) const
{
...
// std::pair<int,int> GenStdpar::GausDesc::subsample_range(int nsamples, double xmin, double xmax, double nsigma) const
// {
...
GenStdpar::GaussianDiffusion::GaussianDiffusion(const IDepo::pointer& depo,
...
void GenStdpar::GaussianDiffusion::set_sampling(const Binning& tbin, // overall time tick binning
...
void GenStdpar::GaussianDiffusion::set_sampling(
                                                ...
                                                unsigned int weightstrat)
{
    ...
    if (!ntss) {
        cerr << "GenStdpar::GaussianDiffusion: no time bins for [" << tval_range.first/units::us << "," << tval_range.second/units::us << "] us\n";
        return;
    }
...
*/
void GenStdpar::GaussianDiffusion::set_sampling_pre(const int diff_idx,
                                                    int & patch_size ,
                                                    double* p_vecs,
                                                    double* t_vecs,
                                                    double* charges, 
...
{
    if (m_patch.size() > 0) {
        cerr << "GenStdpar::GaussianDiffusion: patch size= 0 , diff number:" << diff_idx << "\n";
...
void GenStdpar::GaussianDiffusion::clear_sampling(){
  m_patch.resize(0,0); 
  m_qweights.clear();
...
// patch(row,col)
const GenStdpar::GaussianDiffusion::patch_t& GenStdpar::GaussianDiffusion::patch() const
{
    return m_patch;
}

const std::vector<double> GenStdpar::GaussianDiffusion::weights() const
{
    return m_qweights;
}
